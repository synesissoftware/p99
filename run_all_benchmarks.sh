...
Flags/options:
    ...
    -M
    --no-make
        does not execute make before running benchmarks
...
if [ $RunMake -ne 0 ]; then
  ...
  if [ $ListOnly -eq 0 ]; then
    ...
    echo "Executing build (via command \`$MakeCmd\`) and then running all benchmark programs"
...
if [ $status -eq 0 ]; then
  ...
  if [ $ListOnly -ne 0 ]; then
    ...
    echo "Listing all benchmark programs"
  else
